    ...
    def _setup_data(self, scheme, groups, batch_size, max_seq_length, preprocess):
        ...
        for field_key, field_info in scheme.items():
            ...
            if episode_const:
                ...
                if field_key == 'last_density_update_t_env':
                    self.data.transition_data[field_key] = th.full(
                        (batch_size, *shape), -99999, dtype=dtype, device=self.device)
                else:
...
class ReplayBuffer(EpisodeBatch):
    ...
    def sample(self, batch_size, need_index=False):
        assert self.can_sample(batch_size)
        if self.episodes_in_buffer == batch_size:
            if need_index:
                return np.arange(0, batch_size), self[:batch_size]
            else:
