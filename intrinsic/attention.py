...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None): # (inputs, inputs, inputs)
        ...
        q = self.w_qs(q).view(sz_b, len_q, n_head, d_k)     # X x WQ = Q
        k = self.w_ks(k).view(sz_b, len_k, n_head, d_k)     # X x WK = K
        v = self.w_vs(v).view(sz_b, len_v, n_head, d_v)     # X x WV = V
